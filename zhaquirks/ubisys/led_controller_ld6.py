...
class UbisysLD6SetupCluster(UbisysCluster):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.on_event(AttributeReadEvent.event_type, self._handle_output_event)
        self.on_event(AttributeReportedEvent.event_type, self._handle_output_event)
        self.on_event(AttributeUpdatedEvent.event_type, self._handle_output_event)
        self.on_event(AttributeWrittenEvent.event_type, self._handle_output_event)
...
class UbisysLD6OutputConfigCluster(LocalDataCluster):
    ...
    async def apply_custom_configuration(self, *args, **kwargs):
        """Read the device's current output configuration to sync the local enum.

        The read triggers an AttributeReadEvent on the setup cluster, which
        the UbisysLD6SetupCluster event listener handles to update our cache.
        """
